...
function createPlugin({ name, development }) {
	return (0, _helperPluginUtils.declare)((_, options) => {
		const {
			...
			filter,
			runtime: RUNTIME_DEFAULT = development
				? 'automatic'
				: 'classic',
					...
					enter(path, state) {
						...
						if (file.ast.comments) {
							for (const comment of file.ast.comments) {
								...
								const jsxMatches = JSX_ANNOTATION_REGEX.exec(
									comment.value,
								)
								if (jsxMatches) {
									pragma = jsxMatches[1]
				...
				if (runtime !== 'automatic') {
					...
					if (!useSpread) {
						...
						if (props.length > start) {
							objs.push(
								_core.types.objectExpression(props.slice(start)),
							)
						}
