...
`./babel-preset.js`, composes it with standalone's built-in TypeScript
preset (`isTSX: true, allExtensions: true`), and registers the result
via `Babel.registerPreset('pota', ...)`.

Rollup bundles the preset, then prepends `@babel/standalone/babel.js`
as a banner. The result is a single IIFE file
(`babel-preset-standalone.js`) with zero dependencies.
