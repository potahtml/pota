...
await test(
	'signal write inside a child mounted via reactive trigger reaches long-lived subscribers',
	expect => {
...
await test(
	'signal write inside a replacement child after a sibling tree disposes reaches long-lived subscribers',
	expect => {
...
await test(
	'<Head><title>{signal}</title></Head> updates when a sibling tree swaps and the new child writes that signal',
	expect => {
		const which = signal('a')
...
await test(
	'<Head><title>{signal}</title></Head> updates after Route navigation via navigateSync',
	expect => {
		const title = signal('initial')
...
await test(
	'<Head><title>{signal}</title></Head> updates when a Suspense-resolved child writes that signal',
	async expect => {
		const title = signal('initial')
...
await test(
	'<Head><title>{signal}</title></Head> updates after popstate-driven Route navigation (async onLocationChange path)',
	async expect => {
		const title = signal('initial')
