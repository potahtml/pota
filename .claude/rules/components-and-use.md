...
factory** consumed via `use:ref`, not a custom `use:<name>` propsPlugin.
`use:ref` is the single registered lifecycle attribute; `propsPlugin`
registrations for new directive names are deprecated.
...
`bind` is the one intentional holdout. It ships as a `propsPlugin('use:bind', …)`
and `bind(initial)` itself returns a `SignalFunction` — so a single call
covers both the cell and the directive value:
