...
const unwrap = o => untrack(() => JSON.parse(JSON.stringify(o)))
...
describe('Handling functions in state', () => {
	test('Array Native Methods: Array.Filter', () => {
		createRoot(() => {
			...
			expect(getFiltered()).toJSONEqual([1])
		})
	})
...
describe('State wrapping', () => {
	...
	/*	test('Setting non-wrappable', () => {
		const date = new Date()
		const state = createMutable({ time: date })
		// not wrapped
		expect(String(state.time)).toBe(date)
	})*/
	test('Respects batch in array mutate 2', () => {
		...
		batch(() => {
			...
			expect(state.length).toBe(3)
			expect(state).toJSONEqual([2, 1, 3])
		})
		expect(state.length).toBe(3)
		expect(state).toJSONEqual([2, 1, 3])
	})
